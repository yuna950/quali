...
import { ExamReadinessChecklist } from '@/components/home/ExamReadinessChecklist'
  ...
  return (
    <section>
      <div className="mb-3 flex items-center justify-between">
        <h2 className="heading-3">{user ? `${user.name}님의 시험` : '나의 시험'}</h2>
        <QuickAddPlanDialog
          onAdded={handleAdded}
          trigger={
      ...
      {slides && slides.length === 0 && (
        <Card>
          <CardContent className="desc-3 py-10 text-center text-muted-foreground">
                  ...
                  <Link to={`/mypage/records/${plan.jmCd}`}>
                    <Card className="transition-shadow hover:shadow-md">
                      <CardContent className="flex flex-col gap-4">
                        <div className="flex min-h-32 flex-col justify-between gap-6 sm:flex-row">
                          ...
                          <div className="text-right sm:self-end">
                            <p className="desc-5 mb-1 text-muted-foreground">{STAGE_LABEL[plan.stage]} 시험</p>
                            <p className="text-3xl font-extrabold text-brand">{formatDday(plan.examDate)}</p>
                            <p className="desc-5 mt-1 text-muted-foreground">{formatYyyymmdd(plan.examDate)}</p>
                          </div>
                        </div>
                        {diffInDays(plan.examDate) <= 1 && <ExamReadinessChecklist planId={plan.id} />}
                      </CardContent>
                    </Card>
                  </Link>
                </CarouselItem>
